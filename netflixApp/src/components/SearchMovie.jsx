...
function SearchMovie() {
  ...
  return (
    <>
      ...
      {
        searchedMovies?.length > 0 ? <MovieLists title={movieName} movies={searchedMovies} searchingMovie={true} /> : <div className="text-center text-xl text-gray-500">search any other movies...</div>
      }

      {
        allmovies?.length <= 0 ? <h1 className="text-center text-lg text-red-500">{`no movies available for ${movieName}`}</h1> : ""
      }
